...
const addUsers = async function (cohort, updates) {
  ...
  if (!users.length) {
    throw new Error("Each users must exist in data base");
  }

  // check if each provided user exist in db
  const ifEveryExist = updates[field].every((user) =>
    users.find((usr) => usr._id.toString() === user)
  );
  // throw an error if not each lesson id provided is found in db
  if (!ifEveryExist) {
    throw new Error("All users ids provided must exist in the data base");
  }
...
cohortSchema.methods.updateUsers = function (updates) {
  return addUsers(this, updates);
};
