...
const getData = async (tableName, user) => {
  ...
  try {
    ...
    if (tableName === "Projects") {
      ...
      const devRecords = await projectBase("People").select({view: "All developers", filterByFormula: `SEARCH("${currentUserID}",{github})`}).all();
      const devData = JSON.parse(JSON.stringify(devRecords))
      console.log("devData: ", devData)
      const developersTableID = devData.id;
      console.log("ID : ", developersTableID)
      records = await projectBase("Projects").select({ view: "Active", filterByFormula: `SEARCH("${developersTableID}", ARRAYJOIN("Developers", ",")) > 0` }).all()//encodeURIComponent(`{Developers} = ${developersTableID}`) }).all()
     }
